...
___
### fontello
* 이미지(아이콘)를 vector형태로 만들어서 창 크기를 바꿔도 이미지가 깨지지 않음
* 사이트에서 이미지를 선택해 다운로드 받으면 폴더가 생김
* 폴더에서는 이미지에 대응하는 유니코드를 볼 수 있는 html 파일과, css파일이 있음
* 그 다음에 아이콘을 사용할 html파일에 css파일을 import 하고, font를 fontello로 한 뒤에 대응하는 유니코드를 본문에 쓰면 아이콘을 사용 가능함
* 다른 방법으로는 아이콘 태그(i)에 class name을 각 아이콘의 이름으로 해서 사용하는 방법이 있음. 이 때는 font-family를 지정하지 않아도 됨
* 
* [폰텔로사이트](https://fontello.com/)